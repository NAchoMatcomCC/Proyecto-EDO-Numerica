...
@bot.message_handler(state=MyStates.Inic, commands=['isoclina'])
...
@bot.callback_query_handler(state=MyStates.Inic, func= lambda call : VerifyMethod(call.data))
...
async def ZeroMethodSelector(msg):
    ...
    bt5 = tp.InlineKeyboardButton('Metodo de Newton', callback_data='NM')
    markup = tp.InlineKeyboardMarkup()
    markup.add(bt1)
    markup.add(bt2)
    markup.add(bt3)
...
@bot.callback_query_handler(state=MyStates.Inic, func= lambda call : VerifyZeros(call.data))
...
XLeft: Limite Izquierdo en el eje x\n'''
    ...
    if call.data != 'NM':
        text += '''XRight: Limite Derecho en el eje x\n'''
